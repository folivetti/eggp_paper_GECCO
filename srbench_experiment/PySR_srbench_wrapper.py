...
import juliapkg
juliapkg.require_julia("~1.10") # avoid memory leak bug
...
df = pd.read_csv(f"datasets/{sys.argv[1]}_train{sys.argv[4]}.csv")
df_test = pd.read_csv(f"datasets/{sys.argv[1]}_test{sys.argv[4]}.csv")
