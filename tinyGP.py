...
def optimize(individual, ds):
    ...
    def fun(theta):
        return mse(ds, individual, theta)

    sol = None
    for i in range(2):
        sol_t = minimize(fun, t0, options = {'maxiter' : 50}, method='cg')
...
def fitness(individual, ds):
    if individual.size() > MAX_SIZE:
        return -np.inf
    theta = optimize(individual, ds)
    _fit = mse(ds, individual, theta)
...
def main():
    ...
    dataset = pd.read_csv(sys.argv[2]).values #, delimiter="\t").values
    global TERMINALS
    TERMINALS = [f'x{i}' for i in range(dataset.shape[1]-1)] + ['p']
