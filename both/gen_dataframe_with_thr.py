...
parser.add_argument('--tsize', nargs='?', default=20)
...
if grid == "eggp":
    ...
    algs = ["SymRegg","eggp",  "PySIPS", "Operon", "QLattice", "GPGOMEA", "PySR", "neogp"] #, "random"] # "qlattice", "gpzgd", "Random", "RF"]
    ...
    for alg in algs:
        ...
        for i,f in enumerate(glob.glob(directory)):
            try:
                ...
                # assign a low score for expressions larger than the maximum size
                dfi.loc[dfi['size'] > int(args.tsize), "R2_train"] = 0
                dfi.loc[dfi['size'] > int(args.tsize), "R2_test"] = 0
                dfi.loc[dfi['size'] > int(args.tsize), f"{msecol}_train"] = np.inf
                dfi.loc[dfi['size'] > int(args.tsize), f"{msecol}_test"] = np.inf
                dfi.dropna(inplace=True, how='any') # tinyGP sometimes fails
...
outname = f"perf_{grid}_withred.csv" if grid!="" else "perf_withred.csv"
