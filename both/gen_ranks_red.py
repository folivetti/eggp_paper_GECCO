...
df = pd.read_csv(f"perf{'_'+grid if len(grid) else ''}_withred.csv")
#algs = sorted(np.unique(df.algorithm.values))
algs = ["eggp",  "PySIPS", "Operon", "QLattice", "PySR", "neogp", "PySR_red","Operon_red", "eggp_red"]
...
plt.savefig(f'plots/ranks/diagram_red_{args.criteria}_{args.agg}{'_pct' if args.pct else ''}{'_'+grid if len(grid) else ''}{'_'+args.ext if len(args.ext) else args.ext}.eps')
